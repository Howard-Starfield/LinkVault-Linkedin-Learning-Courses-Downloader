...
///
/// When no preferred or fallback language matches, the best remaining eligible
/// track is selected instead of returning `None`. Automatic tracks are omitted
/// unless allowed, uploader tracks beat automatic tracks, and English is
/// preferred when the caller supplied no language at all.
pub fn select_transcript(
    ...
    for language in &languages {
        if let Some(selected) = pick_transcript(
            tracks,
            Some(language),
            allow_automatic_captions,
...
fn pick_transcript(
    ...
    let mut candidates = tracks
        ...
        .filter(|track| {
            required_language.map_or(true, |language| {
                normalize_language(&track.language_tag) == language
            })
        })
        .collect::<Vec<_>>();
    candidates.sort_by(|left, right| transcript_order(left, right, prefer_english));
    candidates.first().map(|track| SelectedTranscript {
...
fn transcript_order(
    ...
    transcript_source_rank(left)
        ...
        .then_with(|| {
            if prefer_english {
                english_rank(left).cmp(&english_rank(right))
            } else {
...
fn english_rank(track: &YouTubeTranscriptTrack) -> u8 {
    if normalize_language(&track.language_tag) == "en" {
        0
    } else {
        1
    }
...
mod tests {
    ...
    #[test]
    fn transcript_selection_picks_automatic_english_without_language_preference() {
        let tracks = vec![
            track(
                "automatic-ja",
                "ja",
        ...
        let selected =
            select_transcript(&tracks, None, &[], true, &["live_chat".to_string()]).unwrap();
        assert_eq!(selected.track_key, "automatic-en");
        assert_eq!(selected.language_tag, "en");
        assert_eq!(selected.source, YouTubeTranscriptSource::Automatic);

        let ja_only = vec![track(
            "automatic-ja",
            "ja",
            YouTubeTranscriptSource::Automatic,
            false,
        ...
        let selected = select_transcript(&ja_only, None, &[], true, &[]).unwrap();
        assert_eq!(selected.track_key, "automatic-ja");
    }

    #[test]
    fn transcript_selection_returns_none_when_automatic_disallowed_without_language_preference() {
        let tracks = vec![track(
            "automatic-en",
            "en",
            YouTubeTranscriptSource::Automatic,
        ...
        assert!(select_transcript(&tracks, None, &[], false, &[]).is_none());
    }
