...
use crate::app::safe_output_filesystem::{HelperTempCapability, HelperTempDirectory};
...
pub(super) fn run(
    ...
    let environment = build_environment_block(capability);
    let application = to_wide_nul(executable.as_os_str());
    let mut command_line = build_command_line(&executable, &spec.args);
    let current_directory = to_wide_nul(capability.root_path().as_os_str());
    ...
    capability
        .revalidate()
        .map_err(super::helper_temp_from_safe)?;
    let created = unsafe {
        CreateProcessW(
            application.as_ptr(),
    ...
    if let Err(error) = capability.revalidate() {
        terminate_suspended_child(&job, &process);
        return Err(super::helper_temp_from_safe(error));
    }
    ...
    if let Err(error) = capability.revalidate() {
        terminate_suspended_child(&job, &process);
        let _ = finish_reader(stdout_task);
        let _ = finish_reader(stderr_task);
        return Err(super::helper_temp_from_safe(error));
    }
...
fn build_environment_block(capability: &HelperTempCapability) -> Vec<u16> {
    ...
    environment.insert(
        "TEMP".to_string(),
        capability.root_path().as_os_str().to_os_string(),
    );
    environment.insert(
        "TMP".to_string(),
        capability.root_path().as_os_str().to_os_string(),
    ...
    environment.insert(
        "DENO_DIR".to_string(),
        capability
            .child_path(HelperTempDirectory::Deno)
            .as_os_str()
            .to_os_string(),
    );
    environment.insert(
    ...
    environment.insert(
        "HOME".to_string(),
        capability
            .child_path(HelperTempDirectory::Home)
            .as_os_str()
            .to_os_string(),
    );
    environment.insert(
    ...
    environment.insert(
        ...
        capability
            .child_path(HelperTempDirectory::LocalAppData)
            .as_os_str()
            .to_os_string(),
    );
    environment.insert(
        "APPDATA".to_string(),
        capability
            .child_path(HelperTempDirectory::RoamingAppData)
            .as_os_str()
            .to_os_string(),
    );
