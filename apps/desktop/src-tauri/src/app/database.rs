//! Shared application database lifecycle and runtime connection policy.
//!
//! Startup owns version checks, verified pre-migration backups, provider
//! schema installation, WAL activation and integrity checks. Ordinary runtime
//! connections apply policy only and never execute schema work. The serialized
//! writer boundary remains a separate Phase 1 checkpoint.
...
pub enum DatabaseLifecycleError {
    ...
    #[error(
        "UNSUPPORTED_SCHEMA_VERSION: database version {found} is newer than supported version {supported}"
    )]
    UnsupportedSchemaVersion { found: i32, supported: i32 },
    #[error("BACKUP_INTEGRITY_FAILED: backup at {path} returned {result}")]
    BackupIntegrityFailed { path: PathBuf, result: String },
    #[error("DATABASE_INTEGRITY_FAILED: database returned {0}")]
...
pub fn initialize_database(
    path: &Path,
) -> std::result::Result<(Connection, DatabaseInitialization), DatabaseLifecycleError> {
    initialize_database_with_diagnostics(path, &DatabaseDiagnostics::default())
}

pub fn initialize_database_with_diagnostics(
    path: &Path,
    diagnostics: &DatabaseDiagnostics,
) -> std::result::Result<(Connection, DatabaseInitialization), DatabaseLifecycleError> {
    let started = std::time::Instant::now();
    let result = initialize_database_inner(path, diagnostics);
    diagnostics.record(DatabaseDiagnosticInput {
        kind: DatabaseDiagnosticKind::Initialization,
        ...
        error_class: result
            .as_ref()
            .err()
            .map(DatabaseLifecycleError::diagnostic_class),
    });
    result
}

fn initialize_database_inner(
    path: &Path,
    diagnostics: &DatabaseDiagnostics,
) -> std::result::Result<(Connection, DatabaseInitialization), DatabaseLifecycleError> {
    ...
    if from_version > CURRENT_SCHEMA_VERSION {
        return Err(DatabaseLifecycleError::UnsupportedSchemaVersion {
            found: from_version,
    ...
    let backup_path =
        if from_version < CURRENT_SCHEMA_VERSION && database_has_application_tables(&connection)? {
            let started = std::time::Instant::now();
            let result =
                create_migration_backup(&connection, path, from_version, CURRENT_SCHEMA_VERSION);
            diagnostics.record(DatabaseDiagnosticInput {
                kind: DatabaseDiagnosticKind::Backup,
                operation: "create_migration_backup",
                provider: DatabaseProvider::App,
                workflow_id: None,
                elapsed: started.elapsed(),
                queue_depth: 0,
    ...
    if from_version < CURRENT_SCHEMA_VERSION {
        ...
        diagnostics.record(DatabaseDiagnosticInput {
            kind: DatabaseDiagnosticKind::Migration,
            ...
            error_class: result.as_ref().err().map(|_| DatabaseErrorClass::Migration),
        });
        result?;
    }
...
impl DatabaseLifecycleError {
    fn diagnostic_class(&self) -> DatabaseErrorClass {
        match self {
            Self::Sqlite(rusqlite::Error::SqliteFailure(code, _)) => match code.code {
                rusqlite::ErrorCode::DatabaseBusy | rusqlite::ErrorCode::DatabaseLocked => {
                    DatabaseErrorClass::Busy
                }
                _ => DatabaseErrorClass::Sqlite,
            },
            Self::Sqlite(_) => DatabaseErrorClass::Sqlite,
            Self::Io(_) => DatabaseErrorClass::Io,
            Self::UnsupportedSchemaVersion { .. } => DatabaseErrorClass::UnsupportedSchema,
            Self::BackupIntegrityFailed { .. } => DatabaseErrorClass::BackupIntegrity,
            Self::DatabaseIntegrityFailed(_) => DatabaseErrorClass::DatabaseIntegrity,
        }
    }
}

pub fn open_runtime(path: &Path) -> Result<Connection> {
    let connection = Connection::open(path)?;
    configure_connection(&connection)?;
    Ok(connection)
}
...
fn database_has_application_tables(connection: &Connection) -> Result<bool> {
...
fn create_migration_backup(
    ...
    from_version: i32,
    to_version: i32,
) -> std::result::Result<PathBuf, DatabaseLifecycleError> {
    let backup_path = unique_backup_path(source_path, from_version, to_version)?;
    ...
    drop(destination);
    verify_backup_file_integrity(&backup_path)?;
...
fn unique_backup_path(
    ...
    let nonce = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    unique_backup_path_with_nonce(source_path, from_version, to_version, nonce)
}

fn unique_backup_path_with_nonce(
    source_path: &Path,
    from_version: i32,
    to_version: i32,
    nonce: u128,
    ...
    let file_name = source_path
        ...
        .unwrap_or("linkvault.sqlite3");
    for suffix in 0..100_u32 {
        let candidate = source_path.with_file_name(format!(
            "{file_name}.pre-migration-v{from_version}-to-v{to_version}-{nonce}-{suffix}.bak"
        ));
        if !candidate.exists() {
            return Ok(candidate);
        }
    ...
    Err(std::io::Error::new(
        ...
        "could not allocate a unique migration backup path",
    ))
}

fn verify_backup_file_integrity(
    backup_path: &Path,
) -> std::result::Result<(), DatabaseLifecycleError> {
    let unreadable = || DatabaseLifecycleError::BackupIntegrityFailed {
        path: backup_path.to_path_buf(),
        result: "unreadable".to_string(),
    };
    let connection =
    ...
    if integrity == "ok" {
        ...
        Err(DatabaseLifecycleError::BackupIntegrityFailed {
            path: backup_path.to_path_buf(),
            result: integrity,
        })
    }
...
fn set_wal_mode(connection: &Connection) -> Result<()> {
    let mode: String =
...
fn verify_database_integrity(
    ...
    if integrity == "ok" {
        ...
        Err(DatabaseLifecycleError::DatabaseIntegrityFailed(integrity))
    }
}
    ...
    for job in active_jobs {
        if job.paused {
            ...
            transition_job_status(
                ...
                restarted_at,
                Some(
                    "Job was active when LinkedVault restarted and was marked failed for recovery.",
                ),
            )?;
            connection.execute(
                r#"
...
mod tests {
    ...
    fn persistence_gate_new_database_initializes_without_backup() {
        ...
        assert_eq!(initialization.backup_path, None);
        assert_eq!(schema_version(&connection).unwrap(), CURRENT_SCHEMA_VERSION);
        assert!(database_has_application_tables(&connection).unwrap());
    ...
    #[test]
    fn persistence_gate_legacy_database_is_backed_up_before_migration() {
        let directory = tempdir().unwrap();
        let db_path = directory.path().join("linkvault.sqlite3");
        ...
        legacy
            .execute_batch(
                "CREATE TABLE legacy_probe (value TEXT NOT NULL);
                 INSERT INTO legacy_probe (value) VALUES ('preserve-me');",
            )
            .unwrap();
        drop(legacy);
        ...
        let backup_path = initialization
            .backup_path
            .expect("populated legacy database must be backed up");
    ...
    fn persistence_gate_runtime_open_does_not_modify_database() {
        ...
        assert_eq!(schema_version(&runtime).unwrap(), CURRENT_SCHEMA_VERSION);
        let table_count: i64 = runtime
            .query_row(
    ...
    #[test]
    fn persistence_gate_future_schema_is_rejected() {
        let directory = tempdir().unwrap();
        let db_path = directory.path().join("linkvault.sqlite3");
        ...
        let error = initialize_database(&db_path).unwrap_err();
    ...
    #[test]
    fn persistence_gate_connection_policy_is_consistent() {
        let directory = tempdir().unwrap();
        let db_path = directory.path().join("linkvault.sqlite3");
        let (connection, _) = initialize_database(&db_path).unwrap();
        drop(connection);
    ...
    #[test]
    fn persistence_gate_unusable_database_path_preserves_the_blocking_file() {
        let directory = tempdir().unwrap();
        let blocking_file = directory.path().join("not-a-directory");
        fs::write(&blocking_file, b"preserve-me").unwrap();
        let db_path = blocking_file.join("linkvault.sqlite3");
    ...
    #[test]
    fn persistence_gate_backup_allocation_never_overwrites_existing_candidates() {
        let directory = tempdir().unwrap();
        let db_path = directory.path().join("linkvault.sqlite3");
        let nonce = 42_u128;
        for suffix in 0..100_u32 {
            let candidate = db_path.with_file_name(format!(
                "linkvault.sqlite3.pre-migration-v0-to-v1-{nonce}-{suffix}.bak"
            ));
            fs::write(candidate, format!("backup-{suffix}")).unwrap();
        }

        let error = unique_backup_path_with_nonce(&db_path, 0, 1, nonce).unwrap_err();
    ...
    #[test]
    fn persistence_gate_corrupt_backup_reports_safe_integrity_failure() {
        let directory = tempdir().unwrap();
        let backup_path = directory.path().join("corrupt.bak");
        fs::write(&backup_path, b"not-a-sqlite-database").unwrap();

        let error = verify_backup_file_integrity(&backup_path).unwrap_err();

        assert!(matches!(
            error,
            DatabaseLifecycleError::BackupIntegrityFailed { path, result }
                if path == backup_path && result == "unreadable"
        ));
    }

    #[test]
    fn persistence_gate_failed_migration_keeps_backup_and_can_retry_idempotently() {
        let directory = tempdir().unwrap();
        let db_path = directory.path().join("linkvault.sqlite3");
        let legacy = Connection::open(&db_path).unwrap();
        legacy
            .execute_batch(
                "CREATE TABLE legacy_probe (value TEXT NOT NULL);
                 INSERT INTO legacy_probe (value) VALUES ('preserve-me');
                 CREATE TABLE newspaper_editions (id TEXT PRIMARY KEY NOT NULL);",
            )
            .unwrap();
        drop(legacy);
        ...
        assert_eq!(schema_version(&failed).unwrap(), 0);
        assert_eq!(
            failed
                .query_row("SELECT value FROM legacy_probe", [], |row| {
                    row.get::<_, String>(0)
                })
                .unwrap(),
            "preserve-me"
        );
        failed.execute("DROP TABLE newspaper_editions", []).unwrap();
        drop(failed);

        let backup_paths = fs::read_dir(directory.path())
            .unwrap()
            .filter_map(std::result::Result::ok)
            .map(|entry| entry.path())
            .filter(|path| path.extension().is_some_and(|value| value == "bak"))
            .collect::<Vec<_>>();
        assert!(!backup_paths.is_empty());
        for backup_path in &backup_paths {
            verify_backup_file_integrity(backup_path).unwrap();
        }
    ...
    #[test]
    fn persistence_gate_competing_initializer_returns_busy_without_migrating() {
        let directory = tempdir().unwrap();
        let db_path = directory.path().join("linkvault.sqlite3");
        let lock_owner = Connection::open(&db_path).unwrap();
        lock_owner
            .execute_batch(
                "CREATE TABLE legacy_probe (value TEXT NOT NULL);
                 INSERT INTO legacy_probe (value) VALUES ('locked');
                 BEGIN IMMEDIATE;",
            )
            .unwrap();
        ...
        assert!(matches!(
            error,
            DatabaseLifecycleError::Sqlite(rusqlite::Error::SqliteFailure(code, _))
                if matches!(
                    code.code,
                    rusqlite::ErrorCode::DatabaseBusy | rusqlite::ErrorCode::DatabaseLocked
                )
        ));
        lock_owner.execute_batch("ROLLBACK").unwrap();
        assert_eq!(schema_version(&lock_owner).unwrap(), 0);
        assert!(database_has_application_tables(&lock_owner).is_ok());
    }

    #[test]
    fn persistence_gate_initialization_emits_safe_structured_events() {
        let directory = tempdir().unwrap();
        let db_path = directory.path().join("linkvault.sqlite3");
        let diagnostics = DatabaseDiagnostics::default();

        initialize_database_with_diagnostics(&db_path, &diagnostics).unwrap();

        let events = diagnostics.snapshot();
        assert!(events
            .iter()
            .any(|event| event.kind == DatabaseDiagnosticKind::Migration));
        assert!(events
            .iter()
            .any(|event| event.kind == DatabaseDiagnosticKind::Initialization));
        assert!(events
            .iter()
            .all(|event| event.outcome == DatabaseDiagnosticOutcome::Ok));
    }
