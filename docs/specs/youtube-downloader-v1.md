...
**Date:** 2026-08-20  
**Status:** Amended Phase 0A draft; renewed internal owner-risk acceptance required before implementation/testing under this specification; public distribution/release blocked pending Y-PUBLIC-REVIEW<br>
...
Paste URL
  ...
  -> verify and publish media/transcript/metadata artifacts
  -> save raw VTT + normalized transcript JSON
  -> record the verified item in the YouTube Downloads catalog
```
...
10. Let users find verified YouTube downloads after route changes and application restart without restoring a transient run.
...
- automatic continuation of an interrupted transient run after cancellation, failure, process exit or application restart;
- arbitrary-disk scanning or a generic artifact repository to populate the YouTube Downloads view.
...
The bridge is non-durable for workflow execution:
...
Phase 0 permits one narrow provider-domain verified-artifact read projection for
YouTube Downloads. It stores only verified manifest identity, display metadata,
safe registered-root locators, artifact summaries, warning summaries and
acquisition time. It MUST NOT become a generic workflow database, job/event
table, scheduler, lease/retry engine or transient-run store. Catalog rows may
be listed after restart, but a row never resumes a run automatically. Deleting
a row never deletes media; deleting media is a separate explicit, confirmed
operation through the safe-filesystem owner.

V1 uses one atomic admission state: `Idle | Discovering(operationId) | Running(runId) | ShuttingDown | Quarantined`. Scan, inspection and start reserve it under one mutex before helper verification or network work, and matching RAII guards prevent stale/abandoned commands from releasing another operation. One runtime-global semaphore caps caption helpers at two. A run executes selected occurrences sequentially with one managed helper tree. Closing the window to the tray does not stop the run; true application exit enters `ShuttingDown`, cancels and joins every managed tree through the app-owned exit barrier. Cleanup that cannot be proven commits `Quarantined` and rejects later work until bounded explicit recovery succeeds. When the shared durable runtime is ready, the provider planner/executor adapters migrate without rewriting provider-domain logic or dual-writing state.
...
The existing disabled `Generic Video` navigation entry is replaced with a mounted `youtube` route and `YouTubeView`. The mounted provider view has **Download** for the transient add/scan/run flow and **Downloads** for the verified catalog. Provider IPC lives behind `src/lib/youtube/ipc.ts`; playlist interpretation, ordering, transcript selection, output paths and execution remain Rust-owned. A YouTube Downloads row is shown only from a verified catalog projection and can be reconciled only against registered output roots and verified manifests.
...
The mounted YouTube provider exposes two tabs:

- **Download**: scan, select, configure and run a new transient download;
- **Downloads**: list verified catalog entries that survive route changes and application restart.

The Downloads tab is a read projection, not a run queue. It never implies that
a cancelled, failed or interrupted run will continue automatically.
...
Flat records commonly omit per-entry availability. An `unknown` row is not
treated as public. When the user selects an unknown row (or it is otherwise
required for a visible inspection/start action), Rust performs a bounded full
metadata lookup through `resolve_youtube_availability` using the same
occurrence and source-plan binding. The response replaces the exact row by
`occurrenceId` and returns its updated availability, typed blocking reason and
metadata digest. Only a positive provider result becomes `available`;
restricted, missing, failed or still-unknown results remain blocked and are
surfaced with a typed reason.
Resolution is bounded by the selected-occurrence and helper-concurrency caps,
and never runs for every unselected playlist entry merely to render the list.
...
After publication, each item shows a separate saved/warning outcome, a safe
display-relative location, and a **Reveal** action. The action uses a backend
artifact locator; React never submits an absolute path or receives a signed
provider URL.
...
An occurrence with flat-scan availability `unknown` remains non-admissible until
the backend resolves it through one bounded full-metadata request. Resolution
is allowed only for selected or explicitly visible unknown occurrences, is
bound to the immutable `scanPlanId`/`occurrenceId`, and is revalidated before
transcript inspection and start. `public`/downloadable becomes `available`;
private, age-gated, removed, unavailable, failed or unresolved results remain
blocked with `VIDEO_UNAVAILABLE` or `PLAYLIST_UNAVAILABLE` as appropriate. The
frontend cannot promote `unknown` by changing a row field.
...
`formatPolicyVersion: 1` builds a deterministic expression from the cap and current metadata. For a bounded height cap it evaluates these candidates in order, with stable source-format tie-breakers:

1. a complete MP4 with H.264 video and AAC audio;
2. separate H.264 video and AAC audio streams that can be merged/remuxed into MP4 without re-encoding;
3. the best available video/audio pair at or below the cap, preferring a deterministic compatible merge container; and
4. the best single-file representation at or below the cap when no pair can be merged.

For `Best`, the height constraint is removed but the same ordering and
tie-breakers apply. A fallback is exceptional behavior, not an alternate
normal policy: if the selected result is VP9, AV1, Opus, WebM or another
non-preferred codec/container, publication may proceed without re-encoding but
the typed outcome records `PLAYBACK_COMPATIBILITY_WARNING` details (safe
container, video codec, audio codec, and why the preferred H.264/AAC/MP4 path
was unavailable). The UI MUST NOT promise OS-native playback for that
fallback, and must distinguish a successfully published warning from failure.
...
Every verified manifest contains exactly one `metadata` entry. `video_only`
contains exactly one media entry and no transcript entries;
`transcript_only` contains exactly one VTT and one normalized-transcript entry
and no media entry; `video_and_transcript` contains exactly one of each media,
VTT and normalized-transcript entry unless the approved
continue-without-transcript policy records the transcript omission. Duplicate
kinds or artifacts outside the mode-specific set fail verification. All listed
relative paths are normalized, root-confined and checksum-verified.

The final item directory may retain the sanitized title chosen at first
publication, but title is presentation metadata only. Existing-item lookup,
partial import and catalog reconciliation MUST resolve by the verified
`artifactFingerprint`/manifest identity and its registered locator, never by
the current provider title or a newly derived directory name. A title change
with unchanged identity updates display metadata without creating a duplicate
artifact directory.

### 15.1 Published locator and verified catalog

Publication returns a typed backend-created locator only after the complete
item directory and manifest have passed verification and atomic publication:

```ts
interface YouTubePublishedArtifactLocator {
  schemaVersion: 1;
  outputRootId: string;
...
interface YouTubeArtifactCatalogEntry {
  schemaVersion: 1;
  catalogEntryId: string;
  artifactFingerprint: string;
  locator: YouTubePublishedArtifactLocator;
  title: string;
  channelName: string | null;
  acquiredAt: string;
  mediaAvailable: boolean;
  transcriptAvailable: boolean;
  warning: YouTubeWarningDetail | null;
  verifiedManifestSha256: string;
...
The catalog is the minimal provider-domain durable read projection permitted by
ADR-003. It is written in one app-owned database transaction after filesystem
publication, contains no run/job/event/lease/retry/schedule state, and is
reconstructed only from registered output roots and verified manifests. Its
stable upsert key is `(outputRootId, relativeItemPath,
verifiedManifestSha256)`; the opaque `catalogEntryId` is derived from or maps
uniquely to that tuple. Copies of the same `artifactFingerprint` in different
registered roots are distinct entries, while repeated publication or
reconciliation of the same tuple is idempotent. A database-sync failure after
publication returns a typed warning and the still-valid publication locator;
later bounded reconciliation may adopt it from the verified manifest. It never
marks unverified files as downloaded. Removing a catalog row never removes
media. Removing media requires a separate explicit confirmation and the
safe-filesystem owner.

`outputRootId` resolves through an app-owned durable output-root registry,
persisted through the existing application migration and `DatabaseWriter`
boundaries. Initial Start may submit a user-selected `outputDir` only for
untrusted root admission: Rust canonicalizes it, rejects replacement/reparse
or volume/identity mismatch, registers the resulting root identity, and then
uses the opaque ID. On restart the registry must reopen and revalidate the root
identity before list, reconcile, reuse or reveal. A missing, moved or replaced
root remains disconnected until the user explicitly reconnects the same
identity; it is never silently rebound by path text.

The catalog is verified-origin rather than permanently verified: it may retain
bounded `missing` or `corrupt` status after revalidation so the UI can explain
why a formerly verified item is unavailable. Those rows confer no access or
reuse authority. `relativeItemPath` is authority-bearing only inside Rust when paired with the
root and verified-manifest identities; React must not interpret or construct
it. `displayRelativePath` is a separate backend-derived display value and
grants no filesystem authority. A `missing` or `corrupt` row is never reusable
or revealable. Only a fresh safe-root and manifest re-verification may return
that projection to `verified`.
...
The app-owned safe-filesystem service supplies a typed helper temp/cache
capability for every helper launch. It creates an unpredictable, exclusively
owned directory tree on the validated application volume, rejects roots and
descendants that are reparse points or identity/volume mismatches, and holds
the directory identities through process creation, execution and bounded
cleanup. `TEMP`, `TMP`, `DENO_DIR`, `HOME`, app cache and FFmpeg/yt-dlp cache
locations all resolve inside that capability; React, provider metadata and
the child process cannot provide or override absolute paths. Cleanup failure
returns a typed error and quarantines the attempt/runtime rather than deleting
...
The adapter uses yt-dlp `--progress-template` and explicit `--print` records with LinkVault-owned prefixes. It never scrapes the normal progress bar. Machine records are framed and parsed incrementally while the helper runs; stdout is never accumulated until process exit. Only complete LinkVault-prefixed records are decoded into enums and bounded numeric fields before entering runtime state. A record/cardinality limit rejects malformed or oversized records, while readers continue draining and discarding bytes after a limit so a noisy helper cannot deadlock the child. Progress delivery is independent of stderr and never exposes raw helper output.

The managed-process port exposes a bounded progress-record callback (or
equivalent stream) to the executor. It reports byte-completed, byte-total and
fraction fields when present, throttles committed non-terminal snapshots to at
most four per second, and emits state/warning/terminal changes immediately.
Long-form downloads additionally have a bounded idle-progress deadline; a
large wall-time allowance is not permission for a wedged helper to run for
...
type YouTubeWarningCode =
  ...
  | "PARTIAL_REUSE_NOT_AVAILABLE"
  | "CATALOG_SYNC_WARNING";

interface YouTubeWarningRecord {
  occurrenceId: string | null;
  detail: YouTubeWarningDetail;
}
...
interface YouTubeItemOutcomeSnapshot {
  ...
  state: YouTubeItemState;
  phase: YouTubeItemPhase;
  warnings: YouTubeWarningDetail[];
  error: YouTubeError | null;
  publishedArtifactKinds: Array<"media" | "vtt" | "transcript_json" | "metadata">;
  publishedArtifact: YouTubePublishedArtifactLocator | null;
  publishedDisplayRelativePath: string | null;
}
...
LinkVault preserves partial files only inside service-controlled staging or a
bounded app-owned partial area. After cancellation or a retryable item failure,
the service may retain only regular, identity-checked partial files with a
validated record containing the full item-artifact projection, helper-lock
digest, format policy, transcript selection, source identity and file hashes.
On a later **new run** the user may explicitly choose to reuse a compatible
partial by setting `reuseCompatiblePartial` in the frozen Start request. The
service selects at most one validated newest candidate per exact artifact
fingerprint and copies it into a new clean attempt before helper launch.
It never resumes in place. Run ID, scan-plan ID, whole-run selection and output
root never participate in compatibility, while artifact fingerprint and all
other listed identity fields must match exactly.

This is manual new-run reuse after cancellation, failure or application
restart, not automatic workflow continuation. LinkVault does not recreate or
advance a transient run after process exit, and a Downloads catalog row never
acts as a resume instruction. Identity-mismatched, malformed, obsolete or
reparse-containing partials are boundedly quarantined and reported as a typed
warning; they are never silently reused or exposed as published artifacts.
When reuse is requested but no compatible candidate exists, the item starts
clean and emits `PARTIAL_REUSE_NOT_AVAILABLE` rather than silently implying
that bytes were resumed.
...
interface YouTubeError {
  ...
  scope: "scan" | "run" | "item" | "catalog" | "reveal";
  retryable: boolean;
  safeMessage: string;
  occurrenceId: string | null;
}

type YouTubeWarningDetail =
  | { code: "PLAYBACK_COMPATIBILITY_WARNING"; safeMessage: string; reason: "preferred_format_unavailable"; container: string; videoCodec: string | null; audioCodec: string | null }
  | { code: "TRANSCRIPT_FALLBACK_USED"; safeMessage: string; reason: "transcript_fallback" }
  | { code: "TRANSCRIPT_MISSING"; safeMessage: string; reason: "transcript_missing" }
  | { code: "ITEM_UNAVAILABLE"; safeMessage: string; reason: "item_unavailable" }
  | { code: "METADATA_DRIFT"; safeMessage: string; reason: "metadata_drift" }
  | { code: "CATALOG_SYNC_WARNING"; safeMessage: string; reason: "catalog_sync" }
  | { code: "PARTIAL_QUARANTINED"; safeMessage: string; reason: "partial_quarantined" }
  | { code: "PARTIAL_REUSE_NOT_AVAILABLE"; safeMessage: string; reason: "partial_reuse_not_available" }
  | { code: "ITEM_FAILED_CONTINUING"; safeMessage: string; reason: "item_continued" }
  | { code: "EXISTING_VERIFIED_REUSED"; safeMessage: string; reason: "existing_verified_reused" }
  | { code: "PLAYLIST_TRUNCATED"; safeMessage: string; reason: "playlist_truncated" };
```

`YouTubeErrorCode` is the following exhaustive closed Rust/TypeScript
discriminant; unknown wire values are rejected.
...
type YouTubeErrorCode =
  ...
  | "HELPER_TEMP_ROOT_INVALID"
  | "ARTIFACT_LOCATOR_INVALID"
  | "CATALOG_SYNC_FAILED"
  | "CATALOG_ENTRY_NOT_FOUND"
  | "OUTPUT_ROOT_DISCONNECTED"
  | "RECONCILE_LIMIT_EXCEEDED"
  | "NATIVE_REVEAL_FAILED"
  | "RUNTIME_QUARANTINED"
  | "CANCELLED"
...
`CATALOG_SYNC_FAILED` with `scope: "catalog"` covers catalog list/reconcile
storage reads and short catalog transactions that fail before a truthful
response can be produced. A post-publication catalog write failure instead
keeps the item successful and uses `CATALOG_SYNC_WARNING` plus its verified
locator. `OUTPUT_ROOT_DISCONNECTED` identifies a missing/replaced registered
root, `RECONCILE_LIMIT_EXCEEDED` identifies a hard request/byte-budget breach,
and `NATIVE_REVEAL_FAILED` is used only after validation succeeds but native
file-manager dispatch fails. None exposes raw SQL, I/O or absolute-path text.
...
`metadata.json` is provider-domain data, not a generic workflow record. It is
required for every verified item (including media-only and transcript-only
items), is generated from revalidated provider facts plus verified FFprobe
data, and is validated before manifest projection. The file is UTF-8 RFC 8785
canonical JSON for this schema version: fixed field names, deterministic
property ordering, no unknown fields, bounded strings/arrays, and no raw
provider URLs beyond the canonical source URL. Its byte length and SHA-256 are
recorded in the manifest. A metadata write, canonicalization or validation
failure prevents publication and catalog insertion.

## 21. Tauri command contract

All helper-running, discovery, transcript-inspection, catalog reconciliation
and reveal commands are async Tauri adapters. They validate closed DTOs and
move blocking helper/process/hash/filesystem work to a controlled blocking
worker (or equivalent runtime-owned executor). Discovery, inspection and Start
reserve transient workflow admission. Catalog list/reconcile/reveal never
reserve or mutate transient run admission: the app persistence/safe-filesystem
owners provide a separate at-most-one reconciliation gate, bounded read
transactions and identity-held locator resolution. No database transaction or
mutex guard may span filesystem scanning, native shell dispatch or `.await`.
Commands MUST NOT perform helper work on the Tauri invoke/main thread or let
React supply a path, executable, argv, subtitle URL or shell fragment. Catalog
listing is bounded and read-only; reveal resolves only a backend-created
locator against its registered root.

The sole path exception is the user-selected `outputDir` on Start. It is an
untrusted one-time root-admission input from the native directory picker, not a
reusable filesystem capability. Rust must validate and register it before any
helper launch; list, reconcile, reuse and reveal accept only opaque registered
root or artifact locators and never path strings.
...
cancel_youtube_download
list_youtube_downloads
reconcile_youtube_downloads
reveal_youtube_artifact
```

Conceptual TypeScript contracts:

```ts
interface GetYouTubeHelperStatusResponse {
  status: "ready" | "blocked";
  code: "HELPER_MISSING" | "HELPER_INTEGRITY_FAILED" | null;
  message: string;
}
...
interface ResolveYouTubeAvailabilityRequest {
  clientOperationId: string;
  scanPlanId: string;
  occurrenceIds: string[]; // unique, selected/visible, backend limit 100
}

interface YouTubeAvailabilityResolution {
  occurrenceId: string;
  videoId: string;
  availability: "available" | "unavailable" | "unknown";
  blockedBy: "VIDEO_UNAVAILABLE" | "PLAYLIST_UNAVAILABLE" | "SCAN_FAILED" | null;
  metadataDigest: string;
}

interface ResolveYouTubeAvailabilityResponse {
  scanPlanId: string;
  occurrences: YouTubeAvailabilityResolution[];
}
...
interface StartYouTubeDownloadRequest {
  ...
  reuseCompatiblePartial: boolean;
}

interface YouTubeStartReceipt {
  clientSubmissionId: string;
  runId: string;
  revision: number;
  scanPlanId: string;
...
interface ListYouTubeDownloadsRequest {
  status?: "verified" | "missing" | "corrupt";
  limit: number; // backend clamps to 100
  cursor: string | null;
}

interface ListYouTubeDownloadsResponse {
  entries: YouTubeArtifactCatalogEntry[];
  nextCursor: string | null;
}

interface ReconcileYouTubeDownloadsRequest {
  outputRootIds: string[]; // unique registered IDs, backend limit 8
  cursor: string | null;
  limit: number; // backend clamps to 100 candidate directories
}

interface ReconcileYouTubeDownloadsResponse {
  adopted: YouTubeArtifactCatalogEntry[];
  missing: YouTubeArtifactCatalogEntry[];
  corrupt: YouTubeArtifactCatalogEntry[];
  nextCursor: string | null;
}

interface RevealYouTubeArtifactRequest {
  locator: YouTubePublishedArtifactLocator;
  target: "item" | "media" | "transcript";
}

interface RevealYouTubeArtifactResponse {
  locator: YouTubePublishedArtifactLocator;
  target: "item" | "media" | "transcript";
  displayRelativePath: string;
  revealed: true;
...
Catalog list order is stable `(acquiredAt DESC, catalogEntryId ASC)` and each
opaque cursor is bound to that order and the request filter. Reconciliation
accepts at most eight unique registered root IDs and processes at most 100
candidate item directories and 16 MiB of aggregate manifest/metadata bytes per
call. `adopted + missing + corrupt` never exceeds the clamped request limit.
When candidates remain it returns a backend-created `nextCursor` bound to the
same ordered root set; exceeding a hard input/byte bound fails with
`RECONCILE_LIMIT_EXCEEDED` before a catalog write. At most one reconciliation
runs at a time, but an active download does not block bounded catalog list or a
locator reveal. Filesystem validation completes before a short
`DatabaseWriter` transaction; neither its mutex nor a database transaction is
held across scanning or `.await`.
...
One mutex-protected state is exactly `Idle`, `Discovering(clientOperationId)`, `Running(runId)`, `ShuttingDown` or `Quarantined`. Scan, availability resolution and transcript inspection atomically change `Idle -> Discovering` before integrity/network work and hold a matching RAII guard; Start atomically validates and clones the selected scan data into a run-owned plan while changing `Idle -> Running`. The operation ID on `resolve_youtube_availability` has the same single-use tombstone and exact `cancel_youtube_discovery` semantics as scan and inspection: cancellation joins its helper before the matching guard may return to `Idle`. A pre-spawn failure either releases the matching discovery reservation or commits the created run to a typed terminal failure; it cannot leave phantom active work. Every pairwise scan/resolution/inspection interleaving, and any of them versus Start or shutdown, returns `RUNTIME_BUSY` without launching a second helper. The single runtime-global caption semaphore permits at most two caption helpers even if an implementation bug attempts multiple calls. Each permit is held through process join/cleanup and released on success, error, cancel, timeout, panic and shutdown; additional selected items remain unspawned in the bounded plan rather than an unbounded permit wait.
...
Start uses a process-lifetime, no-eviction submission ledger capped at 1,024 client IDs. Admission atomically reserves the client submission ID, canonical request fingerprint, run ID and immutable initial `YouTubeStartReceipt` before worker creation; worker-creation failure rolls back the reservation without exposing a reusable run ID. Each entry permanently retains the canonical request fingerprint, run ID and initial receipt. Replaying an identical `clientSubmissionId` returns that byte-equivalent receipt inside a new response envelope with `replayed: true` and never starts work; first acceptance uses `replayed: false`. The receipt always describes initial accepted state `running` and is never rewritten to reflect later progress or terminal state. Current state is retrieved only through `get_youtube_download_state`. Different canonical input returns `SUBMISSION_CONFLICT`. After a newer run makes an older receipt unavailable, replay of that older tombstone returns `SUBMISSION_ID_RETIRED`, never a new run. Reaching the cap returns `SUBMISSION_ID_CAPACITY_EXHAUSTED` until process restart rather than evicting an ID. A new submission ID while a run is active returns `RUN_ALREADY_ACTIVE`. `get_youtube_download_state({ runId: null })` discovers the active run or, when idle, the most-recent run; an explicit ID returns that retained run or `RUN_NOT_FOUND`. This makes the run pausable/cancellable after route reload or response loss while keeping retention bounded.
...
Rust and TypeScript share the same closed unions for run/item states, warning
codes/details, error codes/scopes and published artifact kinds. Unknown wire
values fail deserialization; no frontend DTO uses `code: string` as an escape
hatch. `Quarantined` is a terminal admission condition after process-tree,
filesystem or helper-temp cleanup cannot be proven: all work commands reject
with `RUNTIME_QUARANTINED`, cleanup is retried only through a bounded explicit
restart/recovery path, and no generic incident or scheduler subsystem is
created. Serialization tests cover fresh Start, terminal-before-replay,
identical/conflicting/retired submissions, catalog entries and safe reveal
responses.

`reveal_youtube_artifact` accepts only the opaque locator returned by a
verified publication or catalog entry. Catalog presence is not required: this
keeps a successfully published item revealable when the later catalog
transaction returns `CATALOG_SYNC_WARNING`. The safe-filesystem owner checks
registered output-root identity, relative containment, reparse state, locator
manifest schema/hash and the freshly reopened verified manifest before asking
the app-owned native shell owner to reveal the selected
item/media/transcript in the platform file manager. Success means the native
reveal action was dispatched and returns only the original locator, safe
display-relative path and `revealed: true`; it never returns an absolute path.
Missing, corrupt, disconnected, replaced or identity-mismatched targets fail
closed with a typed error and perform no shell action.
...
- **FR-36:** One atomic admission state MUST serialize scan, availability resolution, transcript inspection, Start and shutdown; a competing operation fails without launching a helper and discovery cancellation targets the exact operation ID for all three discovery operations.
...
- **FR-40:** Internal Y0-Y4 implementation/testing MUST have an affirmative owner-risk acceptance with an explicit scope; public packaging, distribution and release MUST remain blocked until the separate Y-PUBLIC-REVIEW artifact and approved user-facing copy exist.
- **FR-41:** Unknown flat-playlist availability MUST be resolved by a bounded Rust-owned full-metadata step for selected/visible occurrences before inspection/start; React MUST NOT promote `unknown` to available.
- **FR-42:** Media selection MUST use the versioned deterministic H.264/AAC/MP4-first policy, then a typed compatibility-warning fallback without silent re-encoding.
- **FR-43:** Every verified item MUST return a backend-created root-scoped artifact locator and safe display-relative path; reveal/open MUST resolve only that locator through the safe-filesystem owner.
- **FR-44:** Compatibility warnings MUST include understandable typed detail and MUST be shown separately from successful publication; warnings MUST NOT be rendered as raw provider/error tokens alone.
- **FR-45:** Every verified item MUST contain canonical bounded `metadata.json`, and its bytes/hash MUST be included in manifest verification.
- **FR-46:** The provider MUST persist only a minimal verified-artifact catalog read projection, expose bounded list/reconcile and YouTube Download/Downloads tabs, and reconstruct entries after restart without restoring transient runs.
- **FR-47:** Existing-item reuse and partial compatibility MUST resolve by stable manifest/artifact identity, never mutable title or derived display path.
- **FR-48:** Progress records MUST be parsed incrementally through a bounded managed-process stream, with throttled revisions and continued reader draining after limits.
- **FR-49:** Partial reuse MUST be an explicit manual choice in a new run after cancellation, failure or restart; automatic workflow continuation is not supported.
- **FR-50:** Helper temp/cache paths MUST be issued as validated app-owned capabilities and held through execution/cleanup; no command may accept an absolute helper path.
- **FR-51:** Long-running Tauri commands MUST be async adapters with blocking work off the invoke/main thread and no lock guard held across `.await`.
- **FR-52:** Rust/TypeScript DTOs MUST use closed state/error/warning/locator/catalog unions, with atomic bounded replay reservation and fail-closed `Quarantined` admission.
...
- **NFR-16:** Helper-running Tauri commands MUST be async adapters; blocking process, hashing and filesystem work MUST run off the invoke/main thread.
- **NFR-17:** Helper-visible `TEMP`, `TMP`, cache, Deno and FFmpeg paths MUST remain within an identity-held safe capability for the entire attempt and bounded cleanup.
- **NFR-18:** Unknown availability, progress records, metadata, manifests, catalog rows and replay receipts MUST have explicit byte/cardinality limits and fail-closed validation.
- **NFR-19:** The catalog MUST contain only verified provider artifacts and registered-root locators; it MUST NOT add workflow/job/event/lease/retry/schedule tables or transient-run dual writes.
- **NFR-20:** The Downloads tab MUST survive route replacement and application restart, while active run recovery remains in-memory snapshot discovery rather than automatic continuation.
- **NFR-21:** Reuse MUST be independent of mutable title/channel/thumbnail display metadata and MUST use stable artifact/manifest identity.
- **NFR-22:** Every compatibility warning MUST expose safe container/codec/reason details and a successful locator when publication succeeded.
...
### AC-21: Unknown playlist availability

Given a flat-playlist occurrence whose availability is `unknown`

When the user selects it for inspection or download

Then Rust performs a bounded full-metadata resolution bound to the current plan and occurrence

And only a positive downloadable/public result enables the occurrence

And restricted, missing, failed or still-unknown results remain blocked with a typed reason

And changing a frontend row field cannot promote the occurrence.

### AC-22: Deterministic playback fallback and publication locator

Given a selected video with H.264/AAC/MP4 representations

When the requested cap is downloaded

Then the versioned policy deterministically selects the preferred complete or mergeable H.264/AAC/MP4 path without a silent re-encode

And if that path is unavailable, a deterministic fallback publishes with typed safe codec/container/reason details

And the item outcome separately reports successful publication and returns a backend-created safe locator/display-relative path

And a Reveal action resolves only that locator through the registered output root.

### AC-23: Canonical metadata and title-independent reuse

Given a verified media/transcript item

When publication completes

Then canonical bounded RFC 8785 `metadata.json` is written and its bytes/hash are in the verified manifest

And a later provider title change updates display metadata without creating a duplicate for the same artifact fingerprint

And missing, corrupt or mismatched metadata prevents catalog verification.

### AC-24: Restart-visible catalog without workflow continuation

Given a successfully published verified YouTube item

When the route is replaced or the application restarts

Then the YouTube Downloads tab lists the item from the minimal provider-domain catalog with media/transcript/warning summary and Reveal action

And reconcile inspects only registered roots and verified manifests

And no active or interrupted transient run is resumed automatically

And deleting a catalog row does not delete media.

### AC-25: Incremental progress and manual partial reuse

Given an extremely long fragmented download

When bounded LinkVault progress records arrive, the helper is cancelled, fails retryably, or the application restarts

Then progress snapshots update incrementally with bounded memory and throttled revisions

And compatible validated partials can be explicitly imported by a new run into clean staging

And incompatible, malformed or reparse-containing partials are quarantined

And no run continues automatically after cancellation, failure or restart.

### AC-26: Async, safe-capability and closed-contract boundaries

Given helper temp/cache paths, long-running commands, replayed Start requests or unproven cleanup

When the operation executes

Then Tauri adapters remain async and blocking work is off the invoke/main thread

And every helper-visible path comes from an identity-held safe capability

And identical replay returns the original bounded typed receipt without duplicate work

And conflicting/retired/capacity-exhausted submissions fail with closed errors

And cleanup uncertainty enters fail-closed `Quarantined` admission.
...
- deterministic H.264/AAC/MP4 preference, separate-stream merge, VP9/Opus and AV1 fallback warning detail;
- bounded unknown-availability full-metadata resolution and fail-closed restricted/unknown results;
...
- canonical RFC-8785 metadata serialization, field/size limits and metadata/manifest checksum projection;
- closed Rust/TypeScript error, warning, state, locator and catalog DTO serialization.
...
- barrier-controlled pairwise scan/availability-resolution/transcript-inspection interleavings, each discovery operation versus Start, exact cancellation during resolution, and shutdown interleavings;
...
- manual new-run partial import after cancellation, retryable failure and application restart, including no automatic continuation;
- identity-held helper temp/cache capability, root/descendant reparse swaps and bounded cleanup/quarantine;
- incremental framed progress, record/cardinality limits, reader drain after overflow, throttling and idle-progress timeout;
- FFprobe/normalization/atomic-publication failure injection;
- published locator resolution, safe reveal target validation, title drift reuse and catalog-sync failure/adoption;
- catalog migration backup/rollback, list/reconcile boundedness, restart-visible Downloads tab and row-delete-without-media-delete;
...
- Download/Downloads tab navigation, verified catalog loading and safe Reveal action;
- compatibility warning copy with successful-publication state and safe display-relative location;
- unknown-availability resolution loading/blocked/error states and bounded progress records;
...
- canonical metadata/catalog reconstruction after application restart, title drift reuse and manual partial reuse after restart; and
...
- authorize only Y0-Y4 implementation and testing in isolated internal builds; do not authorize public packaging, distribution or release;
...
Gate: internal validation parses the acceptance rather than checking file presence. Exactly one decision must be selected; the product-owner sign-off, scope/copy fields and no-counsel-claim statement must be non-empty; the decision must be unexpired; and the reviewed specification identity must match. The Phase 0A amendment changes the reviewed specification identity, so the prior acceptance is not authorization for this amended draft; implementation/testing under these bytes remains blocked until renewed affirmative acceptance is recorded. A renewed acceptance authorizes Y0-Y4 internal implementation/testing only. It does not authorize public packaging, distribution, release, account access, restricted-content access or helper execution before the Y0 lock/integrity gate passes. Any material reviewed-spec, helper-lock, component, scope or copy change invalidates the acceptance and requires re-acceptance.
...
- output verification;
- pause-after-current-item;
- manual partial reuse behavior;
- app-owned output-root registry and minimal verified-origin YouTube artifact catalog migration;
- safe published locator/reveal contract; and
- Download/Downloads tabs with restart-visible catalog listing.

Gate: format-policy fixtures, FFprobe failure injection, atomic publication,
catalog migration backup/rollback/integrity checks, restart reconstruction,
safe reveal, disk-full cleanup and exact installed media UAT pass. The catalog
contains no workflow/job/event/lease/retry/schedule or active-run state.

### Y4 - adversarial hardening and internal release readiness
...
- unavailable videos;
- installed-app UAT;
- release notices and licensing review; and
- catalog/reconciliation/root-reconnect/partial-reuse hostile failure cases.

Gate: Y4 internal hardening is within a renewed owner-risk acceptance, but
public packaging, distribution and release remain outside it and cannot start
without Y-PUBLIC-REVIEW. All automated gates, mounted UI/accessibility checks,
separate review artifact verification and exact packaged native UAT must pass.
Record the future Phase 2 migration/removal issue for `workflow::transient`;
do not dual-write or pre-create durable workflow/job/event/lease/retry/schedule
tables. The Y3 provider-domain catalog is the only durable YouTube V1 table
exception.
...
Renewed owner-risk acceptance for this amended specification is sufficient to
begin Y0-Y4 internal implementation/testing only. YouTube Downloader V1 is
complete for public release only when Y-PUBLIC-REVIEW is affirmatively
satisfied and an exact clean installed Windows candidate can:
...
23. deterministically prefer H.264/AAC/MP4, publish typed fallback warning details when necessary, and expose a safe backend locator/reveal action;
24. resolve selected unknown playlist availability through bounded Rust-owned metadata lookup without promoting unknown entries in React;
25. publish canonical bounded `metadata.json` and use stable manifest/artifact identity for reuse despite title drift;
26. parse bounded progress records incrementally and provide manual compatible partial reuse in a new run after cancellation, failure or restart, with no automatic continuation;
27. persist only the minimal verified YouTube artifact catalog, reconstruct the YouTube Downloads tab after restart, and keep catalog deletion separate from media deletion; and
28. pass async-command, helper temp/cache capability, atomic bounded replay, closed Rust/TypeScript contract and fail-closed quarantine tests while preserving the internal-only/public-release gates.
